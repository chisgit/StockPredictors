...
def train_model(
    train_ready_data, model_type="linear_regression", X_val=None, y_val=None
):
    """
    ...
    if model_type == "linear_regression":
        print(
            f"Train Model ()LINEAR REGRESSION- Shape of train_ready_data: \n {train_ready_data.shape}\n"
        )
        print(
            f"Train Model ()LINEAR REGRESSION- Columns in train_ready_data: \n{train_ready_data.columns.tolist()}\n"
        )
        ...
        print(
            pd.DataFrame(X_scaled, columns=X.columns, index=X.index).tail()
        ...
        print(
            f"Shape of X_scaled for model training: {X_scaled.shape}"
        )  # Debugging line
        ...
        model = LinearRegression()
        print(
            f"Shape of X_scaled just before model.fit training: {X_scaled.shape}"
        )  # Debugging line
        ...
        coefficients = (
            model.coef_.flatten() if len(model.coef_.shape) > 1 else model.coef_
        )
        ...
        print(
            f"XGBOOST- Columns in train_ready_data: {train_ready_data.columns.tolist()}"
        )
